...
| 내담자 상호작용 | 5개 사례별 공개 사다리, 아바타, 표정·시선·발화 애니메이션 |
| 훈련 모드 | 즉시 코칭이 있는 연습 모드와 지연 피드백 평가 모드 |
| LXD 연습 루프 | 사례 선택 → 3분 집중연습 → 장면별 자기평가 → 근거 비교 → 선택 장면 재연습 |
| 사례 저작 | CaseCatalog와 ScriptableObject에 프로필·아바타·목표·공개 사다리·집중기술·회기 길이 분리 |
...
| 관찰 줌 | 마우스 휠, −/+, 초기화, 얼굴 관찰 프리셋으로 얼굴·자세 관찰 범위 전환 |
| UI 언어 | 상단 `UI: EN / UI: KO` 토글로 고정 UI 문구를 한국어·영어로 전환; 내담자 대사와 상담 입력 원문은 유지 |
| 데이터 기록 | 원본 영상 없이 파생 신호, 정합성, 관계 궤적을 JSONL 기록 |

## 한 회기 진행

브리핑에서 직장 불안, 청소년 학업 압박, 경력 전환과 번아웃, 노년기 사별과 고립, 유학생 소속감과 적응 중 하나를 선택할 수 있습니다. 사례를 바꾸면 내담자 프로필, Rocketbox 아바타, 비언어 해석 규칙, 공개 궤적, 서버 페르소나 키가 함께 바뀝니다.

| 5개 사례 선택 | 얼굴 관찰·시선 진단 |
|---|---|
| ![5개 상담 사례 선택 화면](Screenshots/progress-41-case-selector.png) | ![얼굴 확대와 시선 상태 진단](Screenshots/progress-42-face-observation-debug.png) |
